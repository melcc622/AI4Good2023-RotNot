# -*- coding: utf-8 -*-
"""ResNet18-Model-ver02

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1Wlc3-NlhPDcG8xZ8Gc1eyS8BXVhiCZGX
"""
...
def accuracy(outputs, labels):
  """
  @params:
  `outputs`: The outputs of the model being trained on
  `labels` : The actual labels of the inputs

  @desc:
  Given the outputs (predictions) of the model, the function will return
  an accuracy score (%). First, it will apply argmax to the ouput probabilities,
  classify whether the predicted output is correct or not, then divide it
...
def running_average(data, window):
  """
  @params:
  `data`: data in which we are computing running averages on
  `window`: scope of the moving average

  @desc:
  Calculates the running average of a dataset
  """
  average_data = []
...
def plot_graph(losses, accuracies, info):
  """
  @params:
  `losses`: Penalty of a bad decision
  `accuracies`: The number of correct classifications
  `info`: You may edit to create path in your google drive

  @desc:
  Plots graphs of the accuracy and loss of the "type" and "freshness" classification
  and regression.
...
# # set model name
# model_name = "Tomato/ResNet18_test"
...
# losses, accuracies = train_ResNet18(100,128,1e-4, num_fruits_veg)
# plot_graph(losses, accuracies,info = ["Tomato-ResNet18",100,128,1e-4])
...
# set model name
model_name = "All_Four/ResNet18"
types_fruits_veg = ["Mango", "Strawberry", "Orange", "Banana","Carrot"]
num_fruits_veg = len(types_fruits_veg)
train_data, validate_data, test_data = load_data_custom(types_fruits_veg)
losses, accuracies = train_ResNet18(100,128,1e-4, num_fruits_veg)
plot_graph(losses, accuracies, info = ["All_Four-ResNet18",100,128,1e-4])
...
model = ResNet18(3, 5)
model.load_state_dict(torch.load("/content/drive/Shareddrives/RotNotCrew/models/All_Four/ResNet18-70.pth", map_location=torch.device('cpu')))
model.eval()
...
data_dir = "/content/drive/Shareddrives/RotNotCrew/"
